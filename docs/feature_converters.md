...
Since many drop-in replacement controllers now exist, we've done our best to make them easy to use in existing keyboards.

This page documents the handy automated process for converting builds.
...
| Device                                                                                   | Target            | Argument                        | `rules.mk`                   | Condition                           |
|------------------------------------------------------------------------------------------|-------------------|---------------------------------|------------------------------|-------------------------------------|
...
| [SparkFun Pro Micro - RP2040](https://www.sparkfun.com/products/18288)                   | `promicro_rp2040` | `-e CONVERT_TO=promicro_rp2040` | `CONVERT_TO=promicro_rp2040` | `#ifdef CONVERT_TO_PROMICRO_RP2040` |
...
?> Use at the keyboard level should be limited to configuring low level aspects such as drivers. The end result providing equivalent behavior to the non-converted state.

### Pin Compatibility

To ensure compatibility, provide validation, and power future workflows, a keyboard should declare its `pin compatibility`. This ensures that only valid combinations are attempted. For legacy reasons, this is currently assumed to be `promicro`.

To declare the base interface for conversions, add this line to your keyboard's `rules.mk`:
